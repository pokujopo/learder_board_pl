...
{
    public function __construct(private ReferralService $referral, private RankingService $ranking) {}
    ...
    public function join(Request $r, Game $game) {
        $v=$r->validate(['phone'=>'required|string|max:20','referral_code'=>'required|string|max:255|regex:/^[a-zA-Z0-9_-]+$/','consents'=>'required|array','consents.terms'=>'required|accepted','consents.sms'=>'sometimes|boolean','consents.future_competitions'=>'sometimes|boolean']);
        $key=$r->header('Idempotency-Key'); if(!$key || strlen($key)>128) return response()->json(['status'=>400,'message'=>'Idempotency-Key header is required.'],400);
        $existingKey=IdempotencyKey::where('user_id',$r->user()->id)->where('key',$key)->first(); if($existingKey && $existingKey->response_body) return response()->json($existingKey->response_body,$existingKey->response_status ?? 200);
        $game->refresh(); $now=now(); if(!$game->is_active || !$game->start_date || !$game->end_date || $now->lt($game->start_date) || $now->gt($game->end_date)) return response()->json(['status'=>409,'message'=>'This competition is not currently accepting joins.'],409);
        if($this->normalizePhone($v['phone']) !== $this->normalizePhone($r->user()->phone_number ?? '')) return response()->json(['status'=>422,'message'=>'The phone number must match your account phone number.'],422);
        if(GameUser::where('user_id',$r->user()->id)->where('game_id',$game->id)->where('refercode_verified',true)->exists()) return response()->json(['status'=>409,'message'=>'You are already registered for this competition.'],409);
        if(GameUser::where('game_id',$game->id)->where('refercode',$v['referral_code'])->where('refercode_verified',true)->exists()) return response()->json(['status'=>409,'message'=>'This referral code has already been used.'],409);
    ...
    public function me(Request $r, Game $game) { $p=GameUser::where('user_id',$r->user()->id)->where('game_id',$game->id)->first(); if(!$p) return response()->json(['status'=>404,'message'=>'You have not joined this competition.'],404); return response()->json(['status'=>200,'data'=>['participation'=>$p]]); }
